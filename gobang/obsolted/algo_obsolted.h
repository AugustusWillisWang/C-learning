...
int _ValidPositionForAlgoFinal[BOUNDRY][BOUNDRY];
...
int GenWeightMartix_Algo3()
{
    ...
    // memset(_ValidPositionForAlgoFinal, 1, sizeof(int) * BOUNDRY * BOUNDRY);
    //1(char) makes the array so ugly.....
    for (int a = 0; a < BOUNDRY;a++){
        for (int b = 0; b < BOUNDRY;b++){
            _ValidPositionForAlgoFinal[a][b] = 1;
        }
    }
        for (int a = 0; a < BOUNDRY; a++)
        {
            for (int b = 0; b < BOUNDRY; b++)
            {
                if (board[a][b])
                {
                    for (int tempa = a - 1; tempa <= a + 1; tempa++)
                    {
                        ...
                        {
                            if (!board[BoundLim(tempa)][BoundLim(tempb)])
                                _ValidPositionForAlgoFinal[BoundLim(tempa)][BoundLim(tempb)] = 1;
                        }
                    }
                }
            }
    ...
    for (int a = 0; a < BOUNDRY; a++)
    {
        for (int b = 0; b < BOUNDRY; b++)
        {
            if (_ValidPositionForAlgoFinal[a][b])
            {
                weight[a][b] = GenWeightAt_Algo3(a, b) - _weightnow;
                printf("weightchange%lf\n", weight[a][b]);
            }
            // CK(weight[a][b]);
        }
    }
...
int Show_ValidArray_Algo3()
{
    for (int a = 0; a < BOUNDRY; a++)
    {
        for (int b = 0; b < BOUNDRY; b++)
        {
            printf("%d ", _ValidPositionForAlgoFinal[a][b]);
        }
        puts("");
    }
}
...
int SimpleSituationCheck_Algo3() //Check 4-1 4-2 3-2 in order. If such situation exists, constrain avaliable position to such areas.
{
    ...
    for (int a = 0; a < BOUNDRY; a++)
    {
        for (int b = 0; b < BOUNDRY; b++)
        {
            if (board[a][b])
            {
                _ValidPositionForAlgoFinal[a][b] = 0;
            }
        }
    }
}
...
int GenNaiveConstrain_Algo3(int cntneeded, int spaceneeded, int start, int end, int continuecnt, int *startposition, int *endposition, int incolor)
{
    ...
    if (continuecnt == cntneeded && space == spaceneeded)
    {
        if (_algo3_havenot_clear_array)
        {
            ...
            memset(_ValidPositionForAlgoFinal, 0, sizeof(int) * BOUNDRY * BOUNDRY);
        }
        if (startposition[0] != -1 && startposition[1] != -1)
            _ValidPositionForAlgoFinal[startposition[0]][startposition[1]] = 1;
        if (endposition[0] != -1 && endposition[1] != -1)
            _ValidPositionForAlgoFinal[endposition[0]][endposition[1]] = 1;
        BP;
    }
}
...
double GenWeightFromMarco(int incolor, int cnt, int space, int start0, int end0) //A pattern's contribute;
{
    ...
    case 0:
        ...
        case 1:
            ...
            case 8:
            default:
                return WEIGHT_4_SPACE_0 * result;
            }
        default:
            BP;
        }
